...
class OpsCncL3DefinitionService {
    async getAll() {
      ...
      SELECT
        ...
        Ops_CnC_L3_Description AS description,
        Ops_CnC_L3_Role        AS role,
    ...
    async listRoles() {
        const pool = await getPool();
        const result = await pool.request().query(`
      SELECT DISTINCT Ops_CnC_L3_Role AS role
      FROM GoGreen.OPS.ops_CnC_L3_definition
      WHERE Ops_CnC_L3_Role IS NOT NULL
      ORDER BY Ops_CnC_L3_Role
    `);
    ...
    async create({ role, name, description, enteredOn, enteredBy }) {
      ...
      INSERT INTO GoGreen.OPS.ops_CnC_L3_definition
        (Ops_CnC_L3_Role, Ops_CnC_L3_Name, Ops_CnC_L3_Description, EnteredOn, EnteredBy, EditedOn, EditedBy)
      OUTPUT INSERTED.*
      VALUES
    ...
    async update(id, body) {
        ...
        const result = await request.query(`
      UPDATE GoGreen.OPS.ops_CnC_L3_definition
      SET
        Ops_CnC_L3_Role = @role,
        Ops_CnC_L3_Name = @name,
        Ops_CnC_L3_Description = @description,
        EnteredOn = @enteredOn,
        EnteredBy = @enteredBy,
        EditedOn = @editedOn,
        EditedBy = @editedBy
      OUTPUT INSERTED.*
      WHERE Ops_CnC_L3_Id = @id
    `);
    ...
    async delete(id) {
        ...
        const result = await request.query(`
      DELETE FROM GoGreen.OPS.ops_CnC_L3_definition
      OUTPUT DELETED.*
      WHERE Ops_CnC_L3_Id = @id
    `);
    ...
    async ensureNoDuplicate({ id, role, name }) {
        ...
        const result = await request.query(`
      SELECT TOP 1 Ops_CnC_L3_Id AS ID
      FROM GoGreen.OPS.ops_CnC_L3_definition
      WHERE
        Ops_CnC_L3_Id <> @id
        AND Ops_CnC_L3_Role = @role
        AND Ops_CnC_L3_Name = @name
    `);

        if (result.recordset.length) {
            const err = new Error("Duplicate CnC L3 definition");
            err.code = "DUPLICATE_CNC_L3";
...
export const opsCncL3DefinitionService = new OpsCncL3DefinitionService();